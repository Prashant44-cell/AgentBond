...
export interface VerifyTaskRequestBody {
  task: string;
  content: string;
  sourceUrl?: string;
}
...
export async function handleVerifyTaskRequest(c: Context) {
  ...
  try {
    ...
    let hfClassification: { label: string; score: number }[] | null = null;
      ...
      try {
        ...
        if (hfResponse.ok) {
          ...
          if (hfData && Array.isArray(hfData.labels) && Array.isArray(hfData.scores)) {
            hfClassification = hfData.labels.map((label: string, index: number) => ({
              label,
        ...
        console.warn('  ⚠️ Hugging Face call failed or timed out, continuing with fallback rules:', (hfErr as Error).message);
      }
    }
 ...
 */
export function evaluateRiskAlgorithm(
  task: string,
  content: string,
  sourceUrl?: string,
  hfScores?: { label: string; score: number }[] | null
): VerificationResult {
  // 1. Normalize task and content
  ...
  if (
    combinedText.includes('bank details changed') ||
  ...
  if (
    combinedText.includes('does not match') ||
  ...
  if (
    combinedText.includes('90% discount') ||
  ...
  if (
    combinedText.includes('no refund') ||
  ...
  const hasSuspiciousUrl =
    (sourceUrl && (sourceUrl.includes('.xyz') || sourceUrl.includes('.top') || sourceUrl.includes('.biz') || sourceUrl.includes('bit.ly') || sourceUrl.includes('tinyurl'))) ||
  ...
  if (
    combinedText.includes('external payment') ||
  ...
  if (
    combinedText.includes('no seller address') ||
