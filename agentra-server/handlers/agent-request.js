...
exports.handleCreateAgentRequest = handleCreateAgentRequest;
exports.handleGetAgentRequests = handleGetAgentRequests;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const crypto_1 = __importDefault(require("crypto"));
const CSV_FILE_PATH = path_1.default.join(process.cwd(), 'agent_requests.csv');
...
function initializeCsvFile() {
    if (!fs_1.default.existsSync(CSV_FILE_PATH)) {
        ...
        fs_1.default.writeFileSync(CSV_FILE_PATH, headers + '\n', 'utf-8');
 ...
 */
async function handleCreateAgentRequest(c) {
    try {
        initializeCsvFile();
        const body = await c.req.json().catch(() => ({}));
        const { agentName, workflowRisk, verificationChecks, proposedFeeUsdc, organization, walletAddress, contactEmail, } = body;
        if (!agentName || !workflowRisk) {
            return c.json({ error: 'Agent Name and Workflow Risk Description are required.' }, 400);
        }
        const requestId = `REQ-${Date.now().toString().slice(-6)}-${crypto_1.default.randomBytes(2).toString('hex').toUpperCase()}`;
        ...
        fs_1.default.appendFileSync(CSV_FILE_PATH, row + '\n', 'utf-8');
        ...
        return c.json({ error: 'Failed to record agent request', details: err.message }, 500);
    }
}
/**
 ...
 */
async function handleGetAgentRequests(c) {
    try {
        initializeCsvFile();
        const content = fs_1.default.readFileSync(CSV_FILE_PATH, 'utf-8');
