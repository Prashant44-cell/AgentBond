...
exports.handleEmailPhishingRequest = handleEmailPhishingRequest;
exports.handleProductVerifierRequest = handleProductVerifierRequest;
exports.handlePolicyRiskRequest = handlePolicyRiskRequest;
exports.handleOcrExtractorRequest = handleOcrExtractorRequest;
exports.handlePromptGuardRequest = handlePromptGuardRequest;
exports.handlePrivacyAuditRequest = handlePrivacyAuditRequest;
const auth_1 = require("./auth");
/**
 * 1. POST /verify-email (Email Phishing & Spoofing Analyzer)
 */
async function handleEmailPhishingRequest(c) {
    try {
        ...
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.email_phish', '/verify-email', content, decision, risk, confidence);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'Email analysis failed', details: err.message }, 500);
    }
}
/**
 * 2. POST /verify-product (Product & Marketplace Verifier)
 */
async function handleProductVerifierRequest(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const callerWallet = c.req.header('x-wallet-address') || 'LGMP4QUQ5RB553RZEP6TQHUQDOBJCDOYXHT5AGCAKB6B4TIE5IXA5HMKZA';
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.product', '/verify-product', content, decision, risk, confidence);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'Product verification failed', details: err.message }, 500);
    }
}
/**
 * 3. POST /verify-policy (Policy Risk Summarizer)
 */
async function handlePolicyRiskRequest(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const callerWallet = c.req.header('x-wallet-address') || 'LGMP4QUQ5RB553RZEP6TQHUQDOBJCDOYXHT5AGCAKB6B4TIE5IXA5HMKZA';
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.policy', '/verify-policy', content, decision, risk, confidence);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'Policy risk analysis failed', details: err.message }, 500);
    }
}
/**
 * 4. POST /extract-ocr (OCR Document Extractor)
 */
async function handleOcrExtractorRequest(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const callerWallet = c.req.header('x-wallet-address') || 'LGMP4QUQ5RB553RZEP6TQHUQDOBJCDOYXHT5AGCAKB6B4TIE5IXA5HMKZA';
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.ocr', '/extract-ocr', content, 'safe', 'low', 0.98);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'OCR extraction failed', details: err.message }, 500);
    }
}
/**
 * 5. POST /guard-prompt (Subagent Prompt Injection Guard)
 */
async function handlePromptGuardRequest(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const callerWallet = c.req.header('x-wallet-address') || 'LGMP4QUQ5RB553RZEP6TQHUQDOBJCDOYXHT5AGCAKB6B4TIE5IXA5HMKZA';
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.prompt_guard', '/guard-prompt', content, decision, risk, confidence);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'Prompt guard analysis failed', details: err.message }, 500);
    }
}
/**
 * 6. POST /audit-privacy (PII & GDPR Privacy Leak Validator)
 */
async function handlePrivacyAuditRequest(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const decision = score >= 60 ? 'suspicious' : score >= 25 ? 'review_before_action' : 'safe';
        const callerWallet = c.req.header('x-wallet-address') || 'LGMP4QUQ5RB553RZEP6TQHUQDOBJCDOYXHT5AGCAKB6B4TIE5IXA5HMKZA';
        (0, auth_1.recordVerificationLog)(callerWallet, 'agentra.privacy', '/audit-privacy', content, decision, risk, 0.95);
        return c.json({
            success: true,
        ...
        return c.json({ error: 'Privacy audit failed', details: err.message }, 500);
    }
}
