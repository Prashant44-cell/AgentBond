...
exports.handleMemeGenerateRequest = handleMemeGenerateRequest;
exports.handleMemeStylesRequest = handleMemeStylesRequest;
 ...
 */
function generateMemeText(userPrompt, style, theme) {
    const themeInfo = theme && MEME_CONTEXT.templates[theme]
        ? MEME_CONTEXT.templates[theme]
 ...
 */
function analyzeMemePrompt(prompt, style, textStyle) {
 ...
 */
function transformToGalaxyBrain(text) {
    const transforms = [
        (t) => t.replace(/use/gi, 'UTILIZE'),
        (t) => t.replace(/make/gi, 'SYNTHESIZE'),
        (t) => t.replace(/fix/gi, 'ARCHITECT A SOLUTION FOR'),
        (t) => t.replace(/code/gi, 'ALGORITHMS'),
        (t) => t + ' WITH QUANTUM COMPUTING',
    ];
    let result = text;
 ...
 */
function enhancePromptWithRAG(userPrompt, style, theme, visualStyle) {
    // Theme context
    const themeInfo = theme && MEME_CONTEXT.templates[theme]
        ? MEME_CONTEXT.templates[theme]
    ...
    const visualContext = visualStyle && MEME_CONTEXT.visualStyles[visualStyle]
        ? MEME_CONTEXT.visualStyles[visualStyle]
 ...
 */
async function addTextOverlay(imageBase64, memeText) {
    try {
 ...
 */
async function generateWithMultiModel(enhancedPrompt, HF_API_KEY) {
    ...
    try {
        ...
        const result = await callHuggingFaceModel(AI_MODELS.primary.name, enhancedPrompt, HF_API_KEY, AI_MODELS.primary.steps, AI_MODELS.primary.guidance);
    ...
    try {
        ...
        const result = await callHuggingFaceModel(AI_MODELS.fallback.name, enhancedPrompt, HF_API_KEY, AI_MODELS.fallback.steps, AI_MODELS.fallback.guidance);
    ...
    return await callHuggingFaceModel(AI_MODELS.creative.name, enhancedPrompt, HF_API_KEY, AI_MODELS.creative.steps, AI_MODELS.creative.guidance);
}
/**
 * Call specific Hugging Face model
 */
async function callHuggingFaceModel(modelName, prompt, apiKey, steps, guidance) {
 ...
 */
function generateMockMeme(prompt) {
 ...
 */
async function generateMemeWithHuggingFace(enhancedPrompt, useMultiModel = false) {
    ...
    try {
        ...
        const result = await callHuggingFaceModel(MODEL, enhancedPrompt, HF_API_KEY, modelConfig.steps, modelConfig.guidance);
 ...
 * }
 */
async function handleMemeGenerateRequest(c) {
    try {
 ...
 */
function handleMemeStylesRequest(c) {
    return c.json({
